...
- **Data Filtering**: Optional moving average filter
...
├── config/
│   └── aurora_tracking_config.yaml # Configuration parameters
├── include/
│   └── aurora_ndi_ros2_driver/
...
├── launch/
│   └── aurora_tracking.launch.py   # Launch file with arguments
├── msg/
...
The Aurora driver is configured through the YAML parameter file in `config/aurora_tracking_config.yaml`. The driver supports multiple sensors and can handle both regular sensors (with external ROM files) and autoconfigured tools like NDI pens (with internal ROM).
...
> **Note**: `enable_data_filtering` introduces temporal delay.
...
|----------|---------|-------------|
| `config_file` | `config/aurora_tracking_config.yaml` | Path to configuration YAML file |
...
ros2 run aurora_ndi_ros2_driver aurora_publisher_node --ros-args \
    --params-file ~/ros2_ws/src/aurora_ndi_ros2_driver/config/aurora_tracking_config.yaml
```
